...
public static class SeoExtensions
{
    extension(Forum forum)
    {
        public string GetSeName()
        {
            ...
            return GetSeName(forum.Name);
        }
    }
    extension(ForumGroup forumGroup)
    {
        public string GetSeName()
        {
            ArgumentNullException.ThrowIfNull(forumGroup);
            return GetSeName(forumGroup.Name);
        }
    }
    ...
    public static string GetSeName(string name) => GetSeName(name, false, false);

    // TODO:
    //var seoSettings = DependoResolver.Instance.Resolve<SeoSettings>();
    //return GetSeName(name, seoSettings.ConvertNonWesternChars, seoSettings.AllowUnicodeCharsInUrls);

    public static string GetSeName(string name, bool convertNonWesternChars, bool allowUnicodeCharsInUrls) => name.ToSlugUrl();
